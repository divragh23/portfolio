...
// Component sourced from https://reactbits.dev (InfiniteMenu).
// Adapted for the portfolio:
//   - Items optionally carry a `label` string; if present, a canvas-based
//     texture is generated on the fly so we can use the words "Home /
//     About / Hobbies / Contact" instead of bitmap images.
//   - The action button calls a host-provided `onSelect(item)` callback so
//     the host can smooth-scroll to the matching section instead of opening
//     an external URL.
...
function resizeCanvasToDisplaySize(canvas, maxDpr = 1.5) {
...
class InfiniteGridMenu {
  ...
  _initDiscInstances(count) {
    ...
    for (let j = 0; j < mat4AttribSlotCount; j += 1) {
...
export default function InfiniteMenu({ items = [], scale = 1.0, onSelect, autoStart = true }) {
  ...
  return (
    <div className="infinite-menu" style={{ position: "relative", width: "100%", height: "100%" }}>
      <canvas id="infinite-grid-menu-canvas" ref={canvasRef} aria-label="Portfolio navigation" />

      {activeItem ? (
        <>
          {activeItem.title ? (
            <h2 className={`face-title ${stateClass}`} aria-hidden="true">
              {activeItem.title}
            </h2>
          ) : null}

          {activeItem.description ? (
            <p className={`face-description ${stateClass}`} aria-hidden="true">
              {activeItem.description}
            </p>
          ) : null}
          ...
          <button
            ...
            className={`action-button ${stateClass}`}
            aria-label={`Go to ${activeItem.title || "section"}`}
          >
