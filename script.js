...
function initMusicPlayer() {
  ...
  const meta = root.querySelector(".music-player__meta");
  const playButton = root.querySelector('[data-action="toggle-play"]');
  const nextButton = root.querySelector('[data-action="next-track"]');
  ...
  const tracks = Array.from(root.querySelectorAll(".music-track"));

  if (!audio || !title || !meta || !playButton || !nextButton || !volumeInput || !tracks.length) {
    return;
  }

  let activeIndex = 0;

  function setTrack(index, shouldAutoplay = false) {
    activeIndex = index;
    const activeTrack = tracks[activeIndex];

    tracks.forEach((track, trackIndex) => {
      track.classList.toggle("is-active", trackIndex === activeIndex);
    });

    audio.src = activeTrack.dataset.src || "";
    title.textContent = activeTrack.dataset.title || "Untitled Track";
    meta.textContent = `${activeTrack.dataset.artist || "Unknown Artist"} · ${
      activeTrack.dataset.accent || "Playlist Selection"
    }`;
    progressBar.style.width = "0%";
    currentTime.textContent = "0:00";
    duration.textContent = "0:00";
    playButton.textContent = "Play";
    playButton.setAttribute("aria-label", `Play ${title.textContent}`);
    ...
    if (shouldAutoplay) {
      audio
        ...
        .then(() => {
          ...
          playButton.setAttribute("aria-label", `Pause ${title.textContent}`);
        })
        .catch(() => {
          playButton.textContent = "Play";
        });
  ...
  tracks.forEach((track, index) => {
    track.addEventListener("click", () => {
      const shouldAutoplay = index !== activeIndex || audio.paused;
      setTrack(index, shouldAutoplay);
    });
  });
